    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑随访记录</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑随访记录</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">随访记录信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="jigoushequId" name="jigoushequId" type="hidden">
                                    <div class="form-group  col-md-6">
                                        <label>随访详情</label>
                                        <input id="suifangjiluContentupload" name="file" type="file">
                                        <script id="suifangjiluContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var suifangjiluContentUe = UE.getEditor('suifangjiluContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="suifangjiluContent" id="suifangjiluContent-input">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        $('#suifangjiluContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('suifangjiluContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("suifangjilu/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addsuifangjilu');
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.suifangjiluContent != null && ruleForm.suifangjiluContent != 'null' && ruleForm.suifangjiluContent != '' && $("#suifangjiluContentupload").length>0) {

            var suifangjiluContentUeditor = UE.getEditor('suifangjiluContentEditor');
            suifangjiluContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.suifangjiluContent != null){
                    mes = ''+ ruleForm.suifangjiluContent;
                    mes = mes.replace(/\n/g, "<br>");
                }
                suifangjiluContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#suifangjiluContentupload").length>0) {
            var suifangjiluContentEditor = UE.getEditor('suifangjiluContentEditor');
            if (suifangjiluContentEditor.hasContents()) {
                $('#suifangjiluContent-input').attr('value', suifangjiluContentEditor.getPlainTxt());
            }
        }
    }
    //数字检查
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                insertTime: "required",
                suifangjiluContent: "required",
            },
            messages: {
                laorenId: "老人不能为空",
                jigoushequId: "随访人员不能为空",
                insertTime: "随访时间不能为空",
                suifangjiluContent: "随访详情不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addsuifangjilu = window.sessionStorage.getItem("addsuifangjilu");
        if (addsuifangjilu != null && addsuifangjilu != "" && addsuifangjilu != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("suifangjilu/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#suifangjiluContent").val(ruleForm.suifangjiluContent);
