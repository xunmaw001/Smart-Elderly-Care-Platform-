            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑家人</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="laorenId" name="laorenId" type="hidden">
                                    <div class="form-group col-md-6">
                                        <label>家人姓名</label>
                                        <input id="jiarenName" name="jiarenName" class="form-control"
                                               placeholder="家人姓名">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>家人手机号</label>
                                        <input id="jiarenPhone" name="jiarenPhone" class="form-control"
                                               onchange="jiarenPhoneChickValue(this)"  placeholder="家人手机号">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>家人身份证号</label>
                                        <input id="jiarenIdNumber" name="jiarenIdNumber" class="form-control"
                                               onchange="jiarenIdNumberChickValue(this)"  placeholder="家人身份证号">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>照片</label>
                                        <img id="jiarenPhotoImg" src="" width="100" height="100">
                                        <input name="file" type="file" id="jiarenPhotoupload"
                                               class="form-control-file">
                                        <input name="jiarenPhoto" id="jiarenPhoto-input" type="hidden">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>性别</label>
...
<script>
    ...
    function upload() {
        ...
        <!-- 当前表的文件上传 -->
        $('#jiarenPhotoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= baseUrl + 'file/download?fileName=' + data.result.file;
                document.getElementById('jiarenPhotoImg').setAttribute('src',photoUrl);
                document.getElementById('jiarenPhoto-input').setAttribute('value',photoUrl);
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("jiaren/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addjiaren');
    ...
    //数字检查
        <!-- 当前表的数字检查 -->
        function jiarenPhoneChickValue(e){
        ...
        function jiarenIdNumberChickValue(e){
            var this_val = e.value || 0;
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                laorenId: "required",
                jiarenName: "required",
                jiarenPhone: "required",
                jiarenIdNumber: "required",
                jiarenPhoto: "required",
                sexTypes: "required",
            },
            messages: {
                username: "账户不能为空",
                ...
                laorenId: "老人不能为空",
                jiarenName: "家人姓名不能为空",
                jiarenPhone: "家人手机号不能为空",
                jiarenIdNumber: "家人身份证号不能为空",
                jiarenPhoto: "照片不能为空",
                sexTypes: "性别不能为空",
            }
        }).form();
    ...
    // 获取当前详情
    function getDetails() {
        var addjiaren = window.sessionStorage.getItem("addjiaren");
        if (addjiaren != null && addjiaren != "" && addjiaren != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("jiaren/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#jiarenName").val(ruleForm.jiarenName);
        $("#jiarenPhone").val(ruleForm.jiarenPhone);
        $("#jiarenIdNumber").val(ruleForm.jiarenIdNumber);
    ...
    function showImg() {
        <!--  当前表的图片  -->
        $("#jiarenPhotoImg").attr("src",ruleForm.jiarenPhoto);
    ...
    function readonly() {
        ...
        }else if (window.sessionStorage.getItem('role') == '家人') {
            $(".aaaaaa").remove();
        }else{
            // alert("未知情况.......");
        }
