    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑机构社区人员</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑机构社区人员</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">机构社区人员信息</h3>
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input id="jigoushequPhone" name="jigoushequPhone" class="form-control"
                                               onchange="jigoushequPhoneChickValue(this)"  placeholder="手机号">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>身份证号</label>
                                        <input id="jigoushequIdNumber" name="jigoushequIdNumber" class="form-control"
                                               onchange="jigoushequIdNumberChickValue(this)"  placeholder="身份证号">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>照片</label>
                                        <img id="jigoushequPhotoImg" src="" width="100" height="100">
                                        <input name="file" type="file" id="jigoushequPhotoupload"
                                               class="form-control-file">
                                        <input name="jigoushequPhoto" id="jigoushequPhoto-input" type="hidden">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>性别</label>
...
<script>
    ...
    function upload() {
        ...
        <!-- 当前表的文件上传 -->
        $('#jigoushequPhotoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= baseUrl + 'file/download?fileName=' + data.result.file;
                document.getElementById('jigoushequPhotoImg').setAttribute('src',photoUrl);
                document.getElementById('jigoushequPhoto-input').setAttribute('value',photoUrl);
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("jigoushequ/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addjigoushequ');
    ...
    //数字检查
        <!-- 当前表的数字检查 -->
        function jigoushequPhoneChickValue(e){
        ...
        function jigoushequIdNumberChickValue(e){
            var this_val = e.value || 0;
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                username: "required",
                password: "required",
                jigoushequName: "required",
                jigoushequPhone: "required",
                jigoushequIdNumber: "required",
                jigoushequPhoto: "required",
                sexTypes: "required",
            },
            messages: {
                username: "账户不能为空",
                password: "密码不能为空",
                jigoushequName: "姓名不能为空",
                jigoushequPhone: "手机号不能为空",
                jigoushequIdNumber: "身份证号不能为空",
                jigoushequPhoto: "照片不能为空",
                sexTypes: "性别不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addjigoushequ = window.sessionStorage.getItem("addjigoushequ");
        if (addjigoushequ != null && addjigoushequ != "" && addjigoushequ != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("jigoushequ/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
